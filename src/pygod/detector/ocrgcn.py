...
from . import OCRAPT_DeepDetector
from ..nn import OCRGCNBase
...
class OCRGCN(OCRAPT_DeepDetector):
    ...
    **kwargs
    ...
    def init_model(self, **kwargs):
        ...
        return OCRGCNBase(in_dim=self.in_dim,
    ...
    def validate_model(self, data, visualize=False,fig_title=None):
        # validate at the end of each epoch, not after each batch
        early_stop = 0
        ...
        if visualize:
            fig_title = fig_title.replace("epoch_","validation_epoch_")
        early_stop = self.model.loss_func_val(emb[data.val_mask, :], label=data.y[data.val_mask], threshold = current_threshold, visualize=visualize,fig_title=fig_title)
        del emb, edge_type, edge_index, x, loss, score
        return early_stop
